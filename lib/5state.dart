...
/**
 * 全局状态管理
 * 当应用中需要一些跨部件的状态需要同步是，上面介绍的方法就难以胜任了。
 * 例如：有一个设置页，里面可以设置应用的语言，为了让设置生效，我们的期望是语言状态发生改变时，
 * app中依赖语言的部件能够重新build一下，但是这些依赖应用语言的部件和设置并不在一起，
 * 所以这种情况下上面的方法很难管理，这时，正确的做法是通过一个全局状态管理器来处理这种相据较远的部件之间的通信。
 * 解决办法1:实现一个全局的事件总栈，将语言状态改变对应为一个事件，然后在app中依赖应用语言的组建的 initState 方法中
 *    订阅语言改变的事件。当用户在设置页切换语言后，我们发布语言改变事件，而订阅了此事件的部件就会收到通知，
 *    收到通知后调用 setState() 方法重新 build 一下自身即可。
 * 解决办法2:使用一些专门用于状态管理的包，如 Provider， Redux。
 */
