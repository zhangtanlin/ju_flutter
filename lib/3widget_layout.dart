        ...
        body: Center(
            child: Column(
               ...
               * 水平方向排列其子Widget。
                  Row({
                    ...  
                    TextDirection textDirection,                                       // 表示水平方向子部件的布局顺序（从左往右还是从右往左），
                                                                                          默认为系统当前 Locale 环境的文本方向，如中文英文都是从左往右，而阿拉伯语是从右往左。
                    MainAxisSize mainAxisSize = MainAxisSize.max,                      // MainAxisSize 表示 Row 在主轴（水平）方向上占用的空间，默认是 max ，表示尽可能多的占用水平风向的空间，
                                                                                          此时无论子Widgets实际占用多少水平空间，Row的宽度始终等于水平方向的最大宽度；当子组建没有占满水平剩余空间，
                                                                                          则Row的实际宽度等于所有子部件占用的水平空间。
                    MainAxisAlignment mainAxisAlignment = MainAxisAlignment.start,     // MainAxisAlignment 表示子部件在Row所占用的水平空间内对齐方式，如果 mainAxisSize值为 MainAxisSize.min，则此属性无意义,
                                                                                          只有 mainAxisSize 值为MainAxisSize.max时，此属性才有意义。
                                                                                          MainAxisAlignment.start 表示沿 textDirection 的初始方向对齐，
                                                                                          如果 textDirection 取值为 TextDirection.ltr 时，则 MainAxisAlignment.start 表示左对齐，
                                                                                          如果 textDirection 取值为 TextDirection.rtl 时，则 MainaxisAlignment.start 表示右对齐。
                    ...
                    VerticalDirection verticalDirection = VerticalDirection.down,      // VerticalDirection 表示Row纵轴（垂直）的对齐方向，默认默认是 VerticalDirection.down 表示从上到下。
                    CrossAxisAlignment crossAxisAlignment = CrossAxisAlignment.center, // CrossAxisAlignment 表示子部件在纵轴方向的对齐方式，Row 的高度等于子部件中最高的子元素高度，
                                                                                          他们的取值和 MainAxis一样，包含start/end/center三个值，不同的是 crossAxisAlignment 的参考系是 VerticalDirection，
                                                                                          即 verticalDirection 值为 verticalDirection.down 时 crossAxisAlignment.start 指顶部对齐，
                                                                                          verticalDirection 值为 verticaalDirection.up 时，crossAxisAlignment.start指底部对齐，
                                                                                          而 crossAxisALignment.end 和crossAxisAlignment.start 正好相反。
                    List<Widget> children = const <Widget>[],                          // 子部件数组。
                  })
                */
              Column(
              ...
              /**
               * colum 是在垂直方向上排列子组建，参数和 Row 一样，不同的是布局方向为垂直，主轴纵轴正好相反。
               * 将 Colum 的宽度指定为屏幕宽度，如果想则样做，可以通过ConstrainedBox或SizeBox，来强制更改宽度限制，后面补充。
               * 注意：如果 Column/Row 里面再嵌套 Column/Row 那么只有最外面的 Column/Row 会占用尽可能大的空间，
               *    里面的 Column/Row所占用的空间为实际大小。如果要让里面的 Column 占满外部 Column 可以使用 Expanded 部件。
               */
              Column(
